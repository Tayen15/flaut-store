    ...
    <div class="max-w-7xl p-6 mx-auto rounded-md shadow-md mt-2">
        <form action="{{ route('carousel.update', $carousel->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <div>
                        ...
                        <input id="name" name="name" type="text" value="{{ $carousel->name }}"autocomplete="organization-title" class="block w-full px-4 py-2 mt-2 text-gray-700 bg-white border rounded-md focus:outline-none focus:ring" required>
                    </div>
