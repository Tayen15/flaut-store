        ...
        <form action="{{ route('catalog.update', $catalog->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="grid grid-cols-1 gap-6 mt-4 sm:grid-cols-2">
                <div>
                    ...
                    <input id="price" name="price" type="text" maxlength="8" oninput="javascript: if (this.value.length > this.maxLength) this.value = this.value.slice(0, this.maxLength);" value="{{ $catalog->price }}" autocomplete="price" class="block w-full px-4 py-2 mt-2 text-gray-700 bg-white border rounded-md focus:outline-none focus:ring" required>
                </div>
